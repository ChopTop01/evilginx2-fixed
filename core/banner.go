...
func printUpdateName() {
	...
	txt := nameClr.Sprintf("               - --  Private Edition  -- -")
...
func printOneliner1() {
	...
	txt := textClr.Sprintf("      by ChopTop/RobinHood (") + handleClr.Sprintf("@choptop01") + textClr.Sprintf(")") + spc + textClr.Sprintf("version ") + versionClr.Sprintf("%s", VERSION)
	fmt.Fprintf(color.Output, "%s", txt)
}
